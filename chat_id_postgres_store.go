...
var chatIdPostgresQueries = []string{
	`create table if not exists nurzhas_chat_ids(
		id text,
...
func (c *chatIdStore) Create(ch *ChatId) (*ChatId, error) {
	query := "insert into nurzhas_chat_ids (id, telegram_bot_id, value) values ($1, $2, $3)"
...
func (c *chatIdStore) List(telegramBotId string) ([]ChatId, error) {
	items := []ChatId{}
	query := "select id, telegram_bot_id, value from nurzhas_chat_ids "
